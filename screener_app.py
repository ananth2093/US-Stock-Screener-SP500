# screener_app.py  v9
# ─────────────────────────────────────────────────────────────────────────────
# v9 CHANGES from v8 (sector-adaptive factor weights):
#   1. SECTOR_FACTOR_WEIGHTS + DEFAULT_FACTOR_WEIGHTS replace FACTOR_WEIGHTS
#   2. ROE_PRIMARY_SECTORS constant — Financials uses ROE not ROIC, skips OpMargin
#   3. compute_quality_score() + quality_flag() gain sector= param
#   4. compute_rank_by_sector() pulls per-sector weights (one line)
#   5. render_sector_kpi_panel() shows active weights bar per sector
# All other v8 logic (ROIC computation, Earn Traj, MC% of S&P500,
# FMP bonus layers, Reference Guide) is preserved verbatim.
# ─────────────────────────────────────────────────────────────────────────────
...
# ── CHANGE 1 ──────────────────────────────────────────────────────────────────
# Replaced FACTOR_WEIGHTS with SECTOR_FACTOR_WEIGHTS (11 sectors) +
...
# ── CHANGE 2 ──────────────────────────────────────────────────────────────────
# Financials: standard ROIC is meaningless for banks (ROA ~1-2% by design).
# ROE is the canonical quality metric for financial companies.
# Op Margin is also excluded for Financials (net interest margin is the
# relevant metric but is not surfaced by Yahoo Finance).
# ──────────────────────────────────────────────────────────────────────────────
...
#   Financials → primary metric = ROE (not ROIC), Op Margin excluded.
#   All others → unchanged v8 logic.

def compute_quality_score(roic, roe, int_coverage, op_margin, sector=None):

    scores = []

    # v9: Financials use ROE as primary; all others prefer ROIC then fall back to ROE
    ...
    # v9: skip Op Margin for Financials — not meaningful for banks/insurers
...
def build_screener_table(universe_df, prices_map, merged_map, revenue_map, momentum_map):
    ...
    for _, r in universe_df.iterrows():
        ...
        # CHANGE 3 (call site): pass sector= to quality score
...
# ── Reference Guide ───────────────────────────────────────────────────────────
# render_reference_guide() is preserved verbatim from v8.
# Only the scoring model note inside it is updated for v9.
...
### Quality Score (0–100)
...
*(full v8 content — unchanged, Financials note added below)*

**v9 note — Financials sector:**
For banks, insurance, and diversified financials, the Quality Score uses **ROE** as the primary
profitability metric instead of ROIC, and **Op Margin is excluded** from the composite.
Reason: Bank ROA/ROIC is structurally 1-2% (excellent for a bank, penalised by our ROIC formula).
Net interest margin is the correct margin metric for banks but is not surfaced by Yahoo Finance.
""")

    with tab_peg:
        st.markdown("""### PEG ... *(full v8 content — unchanged)*""")

    with tab_etraj:
        st.markdown("""### Earn Traj ... *(full v8 content — unchanged)*""")

    with tab_mom:
        st.markdown("""### Momentum Score ... *(full v8 content — unchanged)*""")

    with tab_rank:
        st.markdown("""
### Score (0–100)
Composite percentile score within GICS sector using **sector-adaptive weights** (v9).
...
""")
    ...
    with tab_disp:
        st.markdown("""### ROE%, Debt/Eq, Rev Q1-Q4, Data Sources ... *(full v8 content — unchanged)*""")

    st.markdown("""---
**Data sources v9:** Yahoo Finance (primary) · FMP bonus if key available ·
ROIC from Yahoo financials · Earn Traj from Yahoo FwdEPS/TrailEPS ·
MC% computed across all 503 constituents · Sector-adaptive 5-factor scoring
""")
...
# ── APP ENTRY POINT ───────────────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
st.set_page_config(page_title="S&P 500 Screener v9", layout="wide", page_icon="📊")
st.markdown(
...
st.markdown("## S&P 500 Fundamental Screener v9")
st.caption(
...
with page_screener:
    ...
    # CHANGE 3 (call site): pass sector= to quality_flag lambda
    ...
    st.download_button(
        ...
        file_name="sp500_screener_v9_{}.csv".format(datetime.now().strftime("%Y%m%d_%H%M")),
        mime="text/csv",
    )
...
**Score:** Sector-adaptive weights — select a sector to see active weights in the KPI panel.
""")

    st.markdown("---")
    st.caption(
        "Sources v9: Yahoo Finance primary · FMP bonus if key available · "
        "ROIC from Yahoo financials · Earn Traj from Yahoo FwdEPS/TrailEPS · "
        "MC% computed across all 503 constituents · Sector-adaptive scoring"
    )
